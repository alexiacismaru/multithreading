...
RUN echo "CREATE USER spring WITH PASSWORD '1234';" >> /docker-entrypoint-initdb.d/init.sql
RUN echo "CREATE DATABASE threading OWNER spring;" >> /docker-entrypoint-initdb.d/init.sql
RUN echo "GRANT ALL PRIVILEGES ON DATABASE threading TO spring;" >> /docker-entrypoint-initdb.d/init.sql
